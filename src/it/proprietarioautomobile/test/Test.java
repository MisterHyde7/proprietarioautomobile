...
public class Test {
	...
	public static void main(String[] args) {
		...
		try {
			...
			testFindByMinorenni(municipioService, abitanteService);

			testFindByDescrizioneLike(municipioService);

			testFindByCognome(abitanteService);

			testFindByCodiceMunicipio(abitanteService, municipioService);
	...
	private static void testFindByMinorenni(MunicipioService municipioService, AbitanteService abitanteService)
			throws Exception {
		System.out.println(".......testFindByMinorenni inizio.............");

		List<Municipio> listaMunicipiPresenti = municipioService.listAllMunicipi();
		if (listaMunicipiPresenti.isEmpty())
			throw new RuntimeException("testFindByMinorenni fallito: non ci sono municipi a cui collegarci ");

		Abitante minorenne = new Abitante("prova", "prova", 12, "prova");
		Abitante maggiorenne = new Abitante("test", "test", 20, "test");
		Municipio municipioTest = new Municipio("municipio test", "II", "municipio test");
		municipioService.inserisciNuovo(municipioTest);
		minorenne.setMunicipio(listaMunicipiPresenti.get(listaMunicipiPresenti.size() - 1));
		maggiorenne.setMunicipio(listaMunicipiPresenti.get(listaMunicipiPresenti.size() - 1));
		abitanteService.inserisciNuovo(maggiorenne);
		abitanteService.inserisciNuovo(minorenne);

		if (municipioService.cercaTuttiIMunicipiConMinorenni().size() < 1)
			throw new RuntimeException("testFindByMinorenni fallito");

		System.out.println(".......testFindByMinorenni fine: PASSED.............");
	}

	private static void testFindByDescrizioneLike(MunicipioService municipioService) throws Exception {
		System.out.println(".......testFindByDescrizioneLike inizio.............");
		List<Municipio> listaMunicipiPresenti = municipioService.listAllMunicipi();
		if (listaMunicipiPresenti.isEmpty())
			throw new RuntimeException("testFindByMinorenni fallito: non ci sono municipi a cui collegarci ");
		Municipio municipioIniziale = new Municipio("test I", "V", "via vai");
		municipioService.inserisciNuovo(municipioIniziale);
		if (municipioService.findAllByDescrizioneIniziaCon("tes").size() < 0)
			throw new RuntimeException("testFindByMinorenni fallito");
		System.out.println(".......testFindByDescrizioneLike fine: PASSED.............");
	}

	private static void testFindByCognome(AbitanteService abitanteService) throws Exception {
		System.out.println(".......testFindByCognome inizio.............");
		List<Abitante> abitantiEsistenti = abitanteService.listAllAbitanti();
		if (abitantiEsistenti.isEmpty())
			throw new RuntimeException("testFindByCognome fallito: non ci sono abitanti a cui collegarci ");
		Abitante abitanteTest = new Abitante("testiamo", "testiamo", 22, "test");
		abitanteService.inserisciNuovo(abitanteTest);
		if (abitanteService.cercaTuttiConIlCognomeTipo("test").size() < 1)
			throw new RuntimeException("testFindByCognome fallito");
		System.out.println(".......testFindByCognome fine: PASSED.............");
	}

	private static void testFindByCodiceMunicipio(AbitanteService abitanteService, MunicipioService municipioService)
			throws Exception {
		System.out.println(".......testFindByCodiceMunicipio inizio.............");
		List<Abitante> abitantiEsistenti = abitanteService.listAllAbitanti();
		if (abitantiEsistenti.isEmpty())
			throw new RuntimeException("testFindByCodiceMunicipio fallito: non ci sono abitanti a cui collegarci ");
		List<Municipio> listaMunicipiPresenti = municipioService.listAllMunicipi();
		if (listaMunicipiPresenti.isEmpty())
			throw new RuntimeException("testFindByCodiceMunicipio fallito: non ci sono municipi a cui collegarci ");
		Abitante abitanteConCodice = new Abitante("asd", "asd", 10, "asd");
		HashSet<Abitante> listaDiAbitanti = new HashSet<Abitante>();
		listaDiAbitanti.add(abitanteConCodice);
		Municipio municipioPerCodice = new Municipio("test", "VI", "via test");
		municipioPerCodice.setAbitanti(listaDiAbitanti);
		if (abitanteService.findAllByCodiceMunicipioIniziaCon("V").size() < 0)
			throw new RuntimeException("testFindByCodiceMunicipio fallito");
		System.out.println(".......testFindByCodiceMunicipio fine: PASSED.............");
	}
